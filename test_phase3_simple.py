...
def test_phase3_simple():
    ...
    try:
        # 依存関係のインポート
        from datetime import datetime  # Removed timedelta as it's unused
        ...
        print("\n1. Created test data: {}".format(test_data.shape))
        print("   Date range: {} to {}".format(test_data.index[0], test_data.index[-1]))
        ...
        print("   Technical features created: {}".format(len(tech_features)))
        print("   Sample features: {}".format(tech_features[:5]))
        ...
        seasonal_features = [
            ...
            if any(
                x in col.lower()
                for x in ["day_", "month", "quarter", "season", "holiday"]
            )
        ]
        print("   Seasonal features created: {}".format(len(seasonal_features)))
        print("   Sample seasonal features: {}".format(seasonal_features[:5]))

        # 季節性特徴量の値をチェック
        if len(seasonal_df) > 0:
            latest = seasonal_df.iloc[-1]
            print("   Latest values:")
            print("   - Month: {}".format(latest.get("month", "N/A")))
            print("   - Day of week: {}".format(latest.get("day_of_week", "N/A")))
            print("   - Quarter: {}".format(latest.get("quarter", "N/A")))
        ...
        macro_features = [
            ...
            if any(
                x in col.lower() for x in ["rate", "inflation", "gdp", "unemployment"]
            )
        ]
        print("   Macroeconomic features created: {}".format(len(macro_features)))
        print("   Sample macro features: {}".format(macro_features[:5]))
        ...
        print("   Sentiment features created: {}".format(len(sentiment_features)))
        print("   Sample sentiment features: {}".format(sentiment_features[:3]))

        # 6. クロスアセット特徴量のテスト
        print("\n6. Testing cross-asset features...")
        cross_asset_df = feature_engine.create_cross_asset_features(test_data.copy())
        cross_asset_features = [
            col for col in cross_asset_df.columns if "correlation" in col.lower()
        ]
        print("   Cross-asset features created: {}".format(len(cross_asset_features)))
        print("   Sample cross-asset features: {}".format(cross_asset_features[:3]))

        # 7. 季節性要因の計算テスト
        print("\n7. Testing seasonal factor calculation...")
        current_seasonal = feature_engine.calculate_seasonal_factor()
        print("   Current seasonal factor: {:.4f}".format(current_seasonal))
        ...
        print("   Seasonal factors for different dates:")
        for date in test_dates:
            factor = feature_engine.calculate_seasonal_factor(date)
            print("   - {}: {:+.4f}".format(date.strftime("%Y-%m-%d"), factor))
        ...
        print("   Total enhanced features: {}".format(total_features))
        ...
        categories = {
            ...
            "Macro": [
                col
                for col in all_columns
                if any(x in col.lower() for x in ["rate", "gdp", "inflation"])
            ],
            "Sentiment": [
                col
                for col in all_columns
                if any(x in col.lower() for x in ["sentiment", "fear"])
            ],
        ...
        for category, features in categories.items():
            print("   - {}: {}".format(category, len(features)))
        ...
        print("   Total NaN values: {}".format(nan_count))

        # 無限値のチェック
        inf_count = (
            np.isinf(all_features_df.select_dtypes(include=[np.number])).sum().sum()
        )
        print("   Total infinite values: {}".format(inf_count))

        # 定数特徴量のチェック
        numeric_columns = all_features_df.select_dtypes(include=[np.number]).columns
        constant_features = [
            col for col in numeric_columns if all_features_df[col].nunique() <= 1
        ]
        ...
        # 総合スコア計算
        total_score = (
            score_features + score_quality + score_technical + score_seasonal
        ) / 4

        print("\nOverall Score: {:.1%}".format(total_score))
        ...
        print("\nPhase 3 simple test failed: {}".format(e))
        import traceback
