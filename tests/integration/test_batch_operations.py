...
class TestBatchOperations:
    ...
    @patch('yfinance.Ticker')
    async def test_api_batch_update_all(
        self, 
        mock_ticker, 
        mock_multiple_stocks_data
    ):
    ...
    async def test_performance_metrics(
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            result = await cli.batch_add_to_watchlist(
                list(mock_multiple_stocks_data.keys()),
                notes="パフォーマンステスト"
            )
            ...
            # 結果の確認
            assert result.successful_count == 4
            assert result.execution_time_seconds == pytest.approx(execution_time, abs=0.1)
    ...
    async def test_error_recovery_and_rollback(
        ...
        with patch.object(db, 'add_to_watchlist') as mock_add:
            ...
            with pytest.raises(Exception):
                await cli.batch_add_to_watchlist(stock_codes, notes="ロールバックテスト")
