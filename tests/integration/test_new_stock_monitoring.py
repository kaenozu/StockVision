...
class TestNewStockMonitoring:
    ...
    async def test_sony_stock_search_and_monitoring_workflow(
        self, mock_ticker, setup_test_db, mock_yfinance_data
    ):
        """
        完全なワークフローテスト:
        1. 銘柄検索（6758 ソニーグループ）
    ...
    async def test_alert_functionality(self, setup_test_db):
        ...
        # ウォッチリストにアイテム追加
        watchlist_item = WatchlistItem(
            stock_code="6758", alert_high=Decimal("15000"), alert_low=Decimal("12000")
        )
        await db.add_to_watchlist(watchlist_item)

        # 高値アラートのテスト（15100円 > 15000円）
        high_price_stock = StockData(
            stock_code="6758", current_price=Decimal("15100.0")
        )
        ...
        low_price_stock = StockData(stock_code="6758", current_price=Decimal("11900.0"))
