...
def ml_test_config():
    ...
    return {
        ...
        'model_types_to_test': ['random_forest', 'gradient_boosting', 'linear_regression'],
...
def performance_monitor():
    ...
    class PerformanceMonitor:
        ...
        def assert_timing(self, operation, max_time):
            ...
            assert actual_time <= max_time, f"{operation} took {actual_time}s, expected <= {max_time}s"
...
def assert_prediction_result(result):
    ...
    assert result.direction in ['up', 'down', 'stable'], f"Invalid direction: {result.direction}"
