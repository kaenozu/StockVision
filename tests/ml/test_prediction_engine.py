...
class TestFeatureEngine:
    ...
    def test_feature_calculations_accuracy(self, feature_engine, sample_stock_data):
        ...
        pd.testing.assert_series_equal(features['returns'], expected_returns, check_names=False)
        
        # Test SMA calculation
        expected_sma_20 = sample_stock_data['Close'].rolling(window=20).mean()
        pd.testing.assert_series_equal(features['sma_20'], expected_sma_20, check_names=False)
        
        # Test price range calculation
        expected_range = (sample_stock_data['High'] - sample_stock_data['Low']) / sample_stock_data['Close']
        pd.testing.assert_series_equal(features['price_range'], expected_range, check_names=False)
    ...
    def test_feature_completeness(self, feature_engine, sample_stock_data):
        ...
        expected_features = {
            ...
            'technical_indicators': ['macd', 'rsi', 'bb_position', 'stoch_k', 'williams_r', 'atr'],
        ...
        for category, feature_list in expected_features.items():
            for feature in feature_list:
                assert feature in features.columns, f"Missing {category} feature: {feature}"
...
class TestStockPredictionEngine:
    ...
    async def test_model_initialization(self, prediction_engine):
        ...
        assert isinstance(prediction_engine.models[ModelType.RANDOM_FOREST], RandomForestRegressor)
        assert isinstance(prediction_engine.models[ModelType.GRADIENT_BOOSTING], GradientBoostingRegressor)
        assert isinstance(prediction_engine.models[ModelType.LINEAR_REGRESSION], LinearRegression)
        assert isinstance(prediction_engine.models[ModelType.RIDGE_REGRESSION], Ridge)
    
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_train_model_success(self, mock_ticker, prediction_engine, sample_stock_data):
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_predict_price_success(self, mock_ticker, prediction_engine, sample_stock_data):
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_batch_predict(self, mock_ticker, prediction_engine, sample_stock_data):
        ...
        results = await prediction_engine.batch_predict(symbols, PredictionHorizon.DAILY)
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_ensemble_prediction(self, mock_ticker, prediction_engine, sample_stock_data):
        ...
        result = await prediction_engine.get_ensemble_prediction("AAPL", PredictionHorizon.DAILY)
...
class TestModelValidation:
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_model_performance_thresholds(self, mock_ticker, prediction_engine, sample_stock_data):
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_prediction_consistency(self, mock_ticker, prediction_engine, sample_stock_data):
        ...
        prediction1 = await prediction_engine.predict_price("AAPL", PredictionHorizon.DAILY)
        prediction2 = await prediction_engine.predict_price("AAPL", PredictionHorizon.DAILY)
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_feature_importance(self, mock_ticker, prediction_engine, sample_stock_data):
        ...
        if result:
            ...
            valid_features = [
                'returns', 'sma_', 'rsi', 'macd', 'bb_position',
                'momentum_', 'volatility_', 'stoch_', 'williams_r'
            ]
            
            for feature in result.features_used:
                assert any(vf in feature for vf in valid_features), f"Unexpected feature: {feature}"
...
class TestErrorHandling:
    ...
    async def test_invalid_symbol_handling(self, mock_ticker, prediction_engine):
        ...
        prediction = await prediction_engine.predict_price("INVALID", PredictionHorizon.DAILY)
    ...
    async def test_prediction_without_training(self, prediction_engine):
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            result = await prediction_engine.predict_price("NEW_SYMBOL", PredictionHorizon.DAILY)
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_full_workflow(self, mock_ticker, prediction_engine, sample_stock_data):
        ...
        # Step 1: Train model
        train_success = await prediction_engine.train_model(symbol, ModelType.RANDOM_FOREST)
        assert train_success
        
        # Step 2: Make single prediction
        prediction = await prediction_engine.predict_price(symbol, PredictionHorizon.DAILY)
        assert prediction is not None
        assert prediction.symbol == symbol
        
        # Step 3: Get ensemble prediction
        ensemble = await prediction_engine.get_ensemble_prediction(symbol, PredictionHorizon.DAILY)
    ...
    @pytest.mark.asyncio
    @patch('yfinance.Ticker')
    async def test_multiple_model_types(self, mock_ticker, prediction_engine, sample_stock_data):
        ...
        for model_type in model_types:
            ...
            if train_success:
                # Make prediction
                prediction = await prediction_engine.predict_price(symbol, PredictionHorizon.DAILY, model_type)
                if prediction:
