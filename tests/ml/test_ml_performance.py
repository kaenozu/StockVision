...
class TestMLPerformance:
    ...
    async def test_directional_accuracy_threshold(
        self, mock_ticker, realistic_stock_data
    ):
        ...
        for model_type in model_types:
            ...
            if success:
                ...
                if key in engine.model_metrics:
                    ...
                    assert (
                        accuracy > 0.40
                    ), f"{model_type.value} accuracy {accuracy} below threshold"

        # At least one model should achieve reasonable accuracy
        if accuracies:
            assert (
                max(accuracies) > 0.42
            ), "No model achieved reasonable directional accuracy"
    ...
    async def test_model_training_speed(self, mock_ticker, realistic_stock_data):
        ...
        for model_type in [ModelType.RANDOM_FOREST, ModelType.LINEAR_REGRESSION]:
            ...
            if success:
                ...
                assert (
                    training_time < 30
                ), f"{model_type.value} training took {training_time}s"
    ...
    async def test_batch_prediction_efficiency(self, mock_ticker, realistic_stock_data):
        ...
        start_time = time.time()
        # results = await engine.batch_predict(symbols, PredictionHorizon.DAILY) # Commented out as it's unused
        await engine.batch_predict(
        ...
        assert (
            avg_time_per_symbol < 10
        ), f"Batch prediction too slow: {avg_time_per_symbol}s per symbol"
...
class TestModelValidation:
    ...
    def test_metrics_calculation_accuracy(self):
        ...
        assert (
            abs(metrics.mse - mae_squared) / mae_squared < 2
        ), "MSE and MAE relationship seems wrong"
    ...
    def test_directional_accuracy_calculation(self):
        ...
        actual_prices = pd.Series(
            [100, 101, 102, 101, 103, 102, 104, 105, 104, 106], index=dates
        )

        # Predictions that get direction right but values wrong
        y_true = actual_prices[1:]  # Next day prices
        y_pred = np.array(
            [101.5, 102.5, 100.5, 103.5, 101.5, 104.5, 105.5, 103.5, 106.5]
        )
        ...
        ), f"Expected high directional accuracy, got {metrics.accuracy}"
    ...
    async def test_cross_validation_consistency(self, mock_ticker):
        ...
        if len(r2_scores) > 1:
            ...
            assert (
                std_r2 < 0.2
            ), f"R2 scores too inconsistent: {r2_scores}, std={std_r2}"
...
class TestFeatureImportance:
    ...
    async def test_feature_importance_ranking(self, mock_ticker):
        ...
        if result and result.features_used:
            ...
            assert (
                "sma" in feature_names or "ma" in feature_names
            ), "Moving average features should be important"

            # Should have reasonable number of features
            assert (
                1 <= len(result.features_used) <= 5
            ), f"Unexpected number of features: {len(result.features_used)}"
...
class TestModelComparison:
    ...
    async def test_ensemble_vs_individual_models(self, mock_ticker):
        ...
        for model_type in [ModelType.RANDOM_FOREST, ModelType.GRADIENT_BOOSTING]:
            result = await engine.predict_price(
                "TEST", PredictionHorizon.DAILY, model_type
            )
            if result:
        ...
        ensemble_result = await engine.get_ensemble_prediction(
            "TEST", PredictionHorizon.DAILY
        )
        ...
        if ensemble_result and len(individual_predictions) > 1:
            ...
            assert ensemble_result.confidence >= min(
                individual_confidences
            ), "Ensemble confidence too low"
            assert (
                ensemble_result.confidence <= max_individual_confidence * 1.1
            ), "Ensemble confidence suspiciously high"
            ...
            assert (
                min_pred <= ensemble_result.predicted_price <= max_pred
            ), "Ensemble prediction outside individual range"
...
class TestRegressionValidation:
    ...
    def test_model_overfitting_detection(self):
        ...
        assert not (
            high_r2_low_accuracy["r2"] > 0.9 and high_r2_low_accuracy["accuracy"] < 0.5
    ...
    def test_prediction_bounds_validation(self):
        ...
        assert (
            -20 <= change_percent <= 20
        ), f"Predicted change {change_percent}% seems excessive for daily prediction"
