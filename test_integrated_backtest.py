...
def backtest_integrated(stock_codes: List[str], days_back: int = 30) -> Dict:
    """統合予想システムのバックテスト実行"""
    print(f"Starting integrated backtest for {len(stock_codes)} stocks, {days_back} days back...")
        ...
        try:
            ...
            with get_session_scope() as session:
                ...
                for i in range(len(price_data) - test_days, len(price_data) - 1):
                    ...
                    try:
                        # 統合予想を実行
                        result = integrated_predictor.predict(stock_code, use_adaptive_weights=True)
...
def main():
    ...
    for method, metrics in results['by_method'].items():
        print(f"- {method}: {metrics['accuracy']:.1%} accuracy, {metrics['total_predictions']} predictions")
    ...
    for i, pred in enumerate(results['predictions'][:5]):
        ...
        print(f"{i+1}. {pred['stock_code']} ({pred['date']}):")
        print(f"   Current: {current:,.0f}, Predicted: {predicted:,.0f} ({pred_change:+.1f}%), Actual: {actual:,.0f} ({actual_change:+.1f}%)")
        print(f"   Method: {pred['method_used']}, LSTM: {pred['lstm_prediction']:,.0f} ({pred['lstm_weight']:.1%}), Ensemble: {pred['ensemble_prediction']:,.0f} ({pred['ensemble_weight']:.1%})")
