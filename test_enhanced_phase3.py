...
def test_enhanced_phase3_system():
    ...
    try:
        ...
        try:
            ...
            print(f"   Using default/cached values for testing")
        
        # 2. 強化された価格データの取得
        print("\n2. Testing enhanced price data...")
        enhanced_df = enhanced_integrated_predictor.enhance_price_data(test_stock, days=30)
        ...
        if enhanced_df is not None:
            ...
            # 特徴量カテゴリ分析
            technical_features = [col for col in enhanced_df.columns 
                                if any(x in col.lower() for x in ['sma', 'ema', 'rsi', 'macd', 'bb_'])]
            market_features = [col for col in enhanced_df.columns 
                             if any(x in col.lower() for x in ['nikkei', 'topix', 'mothers', 'vix', 'regime'])]
            seasonal_features = [col for col in enhanced_df.columns 
            ...
            if len(enhanced_df) > 0:
                ...
                print(f"   Sample latest features:")
                print(f"   - Close price: {latest_row.get('Close', 'N/A'):,.0f}")
                print(f"   - RSI: {latest_row.get('rsi', 'N/A'):.1f}" if 'rsi' in latest_row else "   - RSI: N/A")
                print(f"   - VIX level: {latest_row.get('vix_level', 'N/A'):.1f}" if 'vix_level' in latest_row else "   - VIX level: N/A")
                print(f"   - Month: {latest_row.get('month', 'N/A')}" if 'month' in latest_row else "   - Month: N/A")
        else:
        ...
        # 基本予想（強化特徴量なし）
        result_basic = enhanced_integrated_predictor.predict_enhanced(test_stock, use_advanced_features=False)
        if result_basic:
        ...
        # 強化予想（高度な特徴量あり）
        result_enhanced = enhanced_integrated_predictor.predict_enhanced(test_stock, use_advanced_features=True)
        if result_enhanced:
            ...
            print(f"   - Enhanced features used: {result_enhanced.enhanced_features_used}")
            print(f"   - Market context score: {result_enhanced.market_context_score:+.3f}")
        ...
        if result_basic and result_enhanced:
            ...
            improvement_percent = (improvement / basic_price) * 100 if basic_price > 0 else 0
            
            confidence_improvement = result_enhanced.confidence - result_basic.confidence
            
            print(f"   Comparison:")
            print(f"   - Price difference: {improvement:+.0f} yen ({improvement_percent:+.2f}%)")
            print(f"   - Confidence improvement: {confidence_improvement:+.1%}")
            print(f"   - Features improvement: {result_enhanced.enhanced_features_used - result_basic.enhanced_features_used}")
            
            # 予想説明の生成
            if hasattr(enhanced_integrated_predictor, 'get_prediction_explanation'):
                explanation = enhanced_integrated_predictor.get_prediction_explanation(result_enhanced)
                ...
                print(f"   - Confidence level: {explanation['summary']['confidence_level']}")
                print(f"   - Market context: {explanation['key_factors']['market_context']}")
                print(f"   - Seasonal effect: {explanation['key_factors']['seasonal_effect']}")
        
        # 5. 特徴量重要度の分析
        print("\n5. Feature importance analysis...")
        
        if result_enhanced and result_enhanced.feature_importance:
            print(f"   Feature category importance:")
