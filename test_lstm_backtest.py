...
from src.ml.lstm_predictor import TENSORFLOW_AVAILABLE, lstm_predictor
...
def backtest_lstm(stock_codes: List[str], days_back: int = 30) -> Dict:
    """LSTMバックテスト実行"""
    print(
        f"Starting LSTM backtest for {len(stock_codes)} stocks, {days_back} days back..."
    )
        ...
        try:
            ...
            with get_session_scope() as session:
                ...
                for i in range(len(price_data) - days_back, len(price_data) - 1):
                    ...
                    # その時点でのデータまでを使って予想
                    # test_data = price_data[:i+1] # Commented out as it's unused
...
def main():
    ...
    print("- Direction Accuracy: {:.1%}".format(overall["accuracy"]))
    print("- Mean Absolute Error: {:.2f}".format(overall["mae"]))
    print("- Root Mean Square Error: {:.2f}".format(overall["rmse"]))
    print("- Total Predictions: {}".format(overall["total_predictions"]))

    print("\nStock-by-Stock Results:")
    for stock, metrics in results["by_stock"].items():
        print(
            "- {}: {:.1%} accuracy, {} predictions".format(
                stock, metrics["accuracy"], metrics["total_predictions"]
            )
        )
    ...
    print("\nAccuracy Assessment:")
    print("- Target: {:.1%}".format(target_accuracy))
    print("- Current: {:.1%}".format(current_accuracy))
    print("- Gap: {:.1%}".format(target_accuracy - current_accuracy))

    if current_accuracy >= target_accuracy:
        print("TARGET ACHIEVED!")
    else:
        improvement_needed = target_accuracy - current_accuracy
        print("Improvement needed: {:.1%}".format(improvement_needed))
