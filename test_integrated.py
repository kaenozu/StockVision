...
def test_integrated_predictor():
    ...
    try:
        ...
        print(f"   Default weights - LSTM: {model_info['integrated']['weights']['lstm']:.1%}, "
              f"Ensemble: {model_info['integrated']['weights']['ensemble']:.1%}")
        
        # 2. 統合予想実行（固定重み）
        print("\n2. Running integrated prediction (fixed weights)...")
        result_fixed = integrated_predictor.predict(test_stock, use_adaptive_weights=False)
        ...
        if result_fixed.lstm_prediction > 0:
            print(f"   LSTM prediction: {result_fixed.lstm_prediction:,.0f} yen (weight: {result_fixed.lstm_weight:.1%})")
        print(f"   Ensemble prediction: {result_fixed.ensemble_prediction:,.0f} yen (weight: {result_fixed.ensemble_weight:.1%})")
        print(f"   Current price: {result_fixed.technical_indicators['current_price']:,.0f} yen")
        
        # 3. 統合予想実行（適応的重み）
        print("\n3. Running integrated prediction (adaptive weights)...")
        result_adaptive = integrated_predictor.predict(test_stock, use_adaptive_weights=True)
        ...
        if result_adaptive.lstm_prediction > 0:
            print(f"   LSTM prediction: {result_adaptive.lstm_prediction:,.0f} yen (adaptive weight: {result_adaptive.lstm_weight:.1%})")
        print(f"   Ensemble prediction: {result_adaptive.ensemble_prediction:,.0f} yen (adaptive weight: {result_adaptive.ensemble_weight:.1%})")
        ...
        print(f"   SMA20: {ti.get('sma_20', 'N/A'):,.0f}" + (" yen" if ti.get('sma_20', 'N/A') != 'N/A' else ""))
        print(f"   RSI: {ti.get('rsi', 'N/A'):.1f}" if ti.get('rsi', 'N/A') != 'N/A' else "   RSI: N/A")
        print(f"   MACD: {ti.get('macd', 'N/A'):.2f}" if ti.get('macd', 'N/A') != 'N/A' else "   MACD: N/A")
        
        # 6. 予想比較
        print("\n6. Prediction comparison...")
        current_price = ti['current_price']
        if result_fixed.predicted_price != result_adaptive.predicted_price:
            print(f"   Fixed weights: {result_fixed.predicted_price:,.0f} yen ({((result_fixed.predicted_price - current_price) / current_price) * 100:+.1f}%)")
            print(f"   Adaptive weights: {result_adaptive.predicted_price:,.0f} yen ({((result_adaptive.predicted_price - current_price) / current_price) * 100:+.1f}%)")
        else:
            print(f"   Both methods: {result_adaptive.predicted_price:,.0f} yen ({((result_adaptive.predicted_price - current_price) / current_price) * 100:+.1f}%)")
