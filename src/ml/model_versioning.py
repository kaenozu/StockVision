...
try:
    ...
    print("MLflow not available. Install with 'pip install mlflow' for full functionality.")
...
class ModelVersionManager:
    ...
    def create_model_version(
        ...
        try:
            ...
            dataset_hash = self._compute_dataset_hash(dataset) if dataset is not None else "unknown"
            ...
            # Save model locally
            local_path = self._save_model_locally(model, metadata)
    ...
    def _save_model_locally(self, model: Any, metadata: ModelMetadata) -> str:
        ...
        try:
            ...
            model_dir = os.path.join(self.config.local_storage_path, metadata.model_name)
    ...
    def load_model(self, model_name: str, version: str) -> Any:
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                logger.info(f"Loaded model {model_name} version {version} from local storage")
                return model
    ...
    def get_production_model(self, model_name: str) -> Optional[Tuple[Any, ModelVersion]]:
        ...
        try:
            ...
            if os.path.exists(metadata_path):
                with open(metadata_path, 'r') as f:
                    ...
                    metadata_dict['trained_at'] = datetime.fromisoformat(metadata_dict['trained_at'])
...
if __name__ == "__main__":
    ...
    # Load model
    if versions:
        model, version_info = model_version_manager.get_production_model("sample_stock_predictor")
        if model:
