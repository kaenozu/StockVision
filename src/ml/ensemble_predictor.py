...
class EnsemblePredictor:
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(self.lookback_days, len(df) - 1):
            # 過去N日分の特徴量
            feature_window = df[feature_columns].iloc[i-self.lookback_days:i].values.flatten()
    ...
    def train_model(self, stock_code: str, force_retrain: bool = False) -> Dict[str, float]:
        ...
        with get_session_scope() as session:
            ...
            if len(price_history) < self.lookback_days + 50:
                raise ValueError(f"Insufficient data for {stock_code}. Need at least {self.lookback_days + 50} days.")
        ...
        logger.info(f"Ensemble model trained for {stock_code}: "
    ...
    def predict(self, stock_code: str) -> EnsemblePredictionResult:
        ...
        with get_session_scope() as session:
            recent_data = session.query(PriceHistory).filter(
                PriceHistory.stock_code == stock_code
            ).order_by(PriceHistory.date.desc()).limit(self.lookback_days + 20).all()
        ...
        # 最新の特徴量ウィンドウ
        latest_features = df[feature_columns].iloc[-self.lookback_days:].values.flatten()
        ...
        # アンサンブル予測
        predicted_price = (model_data['rf_weight'] * rf_pred + 
                          model_data['svm_weight'] * svm_pred)
        ...
        technical_indicators = {
            ...
            'bb_ratio': float(df['bb_ratio'].iloc[-1]) if not pd.isna(df['bb_ratio'].iloc[-1]) else 0.5
        }
