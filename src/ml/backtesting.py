...
class BacktestingEngine:
    ...
    def _get_backtest_timestamps(
        ...
        for symbol in symbols:
            if symbol in self.historical_data:
                ...
                mask = (symbol_data.index >= start_date) & (
    ...
    def open_position(
        ...
        try:
            ...
            logger.info(
                f"Opened {direction.value} position for {symbol}: {quantity} shares at {price}"
            )
    ...
    def close_position(
        self, symbol: str, price: float, order_type: OrderType = OrderType.MARKET
    ):
        ...
        try:
            ...
            logger.info(
                f"Closed {position.direction.value} position for {symbol}: {position.quantity} shares at {price}"
            )
    ...
    def _calculate_metrics(self) -> BacktestResult:
        ...
        value_df = pd.DataFrame(
            self.portfolio.value_history, columns=["timestamp", "value"]
        )
    ...
    def plot_results(self, result: BacktestResult):
        ...
        try:
            ...
            value_df = pd.DataFrame(
                result.portfolio_values, columns=["timestamp", "value"]
            )
            value_df.set_index("timestamp", inplace=True)
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        current_value = (
            self.portfolio.value_history[-1][1]
        ...
        if symbol in market_data and symbol in engine.predictions:
            ...
            if timestamp in pred_data.index:
                ...
                if prediction["predicted_price"] > current_price * 1.01:  # 1% threshold
                    ...
                    prediction["predicted_price"] < current_price * 0.99
                ):  # 1% threshold
                    # Sell signal
