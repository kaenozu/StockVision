...
from ..models.prediction import PredictionModel, PredictionResult, AccuracyHistory
...
class StockMLModel:
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, validation_split: float = 0.2) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Training {self.name} model with {len(X)} samples and {X.shape[1]} features")
        ...
        logger.info(f"Model training completed. Validation R²: {val_metrics['r2_score']:.4f}")
    ...
    def predict(self, X: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if missing_features:
            logger.warning(f"Missing features: {missing_features}. Adding with zero values.")
    ...
    def _calculate_confidence(self, X_scaled: np.ndarray, predictions: np.ndarray) -> np.ndarray:
    ...
    def _calculate_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            'mean_absolute_percentage_error': float(np.mean(np.abs((y_true - y_pred) / y_true)) * 100)
        }
...
class MLModelManager:
    ...
    def train_model(self, model_key: str, stock_data: pd.DataFrame, 
                   target_horizon_days: int = 1) -> Dict[str, Any]:
        """
        ...
        if target_horizon_days == 1:
            ...
            target_col = 'target_return_3d' if target_horizon_days <= 3 else 'target_return_5d'
        else:
        ...
        # Save model
        model_file = os.path.join(self.models_dir, f"{model_key}_v{model.version}.joblib")
    ...
    def predict_ensemble(self, stock_data: pd.DataFrame, 
        ...
        for model_key, model in self.models.items():
            if model.is_trained:
                try:
                    ...
                    val_r2 = model.performance_metrics.get('validation_metrics', {}).get('r2_score', 0.5)
        ...
        weighted_confidence = sum(conf * weights[key] for key, conf in confidences.items()) / total_weight
