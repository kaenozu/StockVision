...
class AdvancedFeatureEngine:
    ...
    def _get_index_data(self, symbol: str, date: datetime) -> Dict:
        ...
        try:
            ...
            return {"current": float(current), "change_percent": float(change_percent)}
    ...
    def _get_sector_performance(self, date: datetime) -> Dict[str, float]:
        ...
        for sector, symbol in sector_etfs.items():
            try:
                ...
                logger.warning(f"Failed to get sector performance for {symbol}: {e}")
        ...
        for date in df.index:
            try:
                ...
                features = {
                    ...
                    "regime_bull": (
                        1 if market_ctx.regime == MarketRegime.BULL_MARKET else 0
                    ),
                    "regime_bear": (
                        1 if market_ctx.regime == MarketRegime.BEAR_MARKET else 0
                    ),
                    "regime_sideways": (
                        1 if market_ctx.regime == MarketRegime.SIDEWAYS else 0
                    ),
                    "regime_high_vol": (
                        1 if market_ctx.regime == MarketRegime.HIGH_VOLATILITY else 0
                    ),
                    "regime_low_vol": (
                        1 if market_ctx.regime == MarketRegime.LOW_VOLATILITY else 0
                    ),
                }
    ...
    def create_seasonal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["is_quarter_end"] = (
            (df["month"].isin([3, 6, 9, 12])) & (df.index.day > 25)
        ).astype(int)
        df["is_year_end"] = ((df["month"] == 12) & (df.index.day > 25)).astype(int)

        # 決算時期効果（日本企業は3月決算が多い）
        df["earnings_season"] = (
            (df["month"].isin([4, 5])) | (df["month"].isin([10, 11]))
        ).astype(int)

        # 夏休み・年末年始効果
        df["summer_holiday"] = (
            (df["month"] == 8) | ((df["month"] == 7) & (df.index.day > 15))
        ).astype(int)
    ...
    def create_macroeconomic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        raise NotImplementedError(
            "Macroeconomic feature fetching is not yet implemented."
        )
