...
class MarketAnomalyDetector:
    ...
    def detect_anomalies(self, stock_data: pd.DataFrame, 
        ...
        logger.info(f"Anomaly detection completed. Level: {anomalies['overall_anomaly_level']}, "
    ...
    def _calculate_baseline(self, data: pd.DataFrame):
        ...
        if len(data) < self.lookback_days:
            logger.warning(f"Insufficient data for baseline calculation: {len(data)} days")
            return
        ...
        baseline_data['log_returns'] = np.log(baseline_data['close'] / baseline_data['close'].shift(1))
        ...
        baseline_data['volatility'] = baseline_data['returns'].rolling(window=20).std() * np.sqrt(252)
        ...
        baseline_data['gap'] = (baseline_data['open'] - baseline_data['close'].shift(1)) / baseline_data['close'].shift(1)
        self.baseline_stats['gap_mean'] = baseline_data['gap'].mean()
        self.baseline_stats['gap_std'] = baseline_data['gap'].std()
    
    def _detect_volatility_anomaly(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        # Z-score calculation
        volatility_z_score = (recent_volatility - self.baseline_stats['volatility_mean']) / \
        ...
        if abs(volatility_z_score) > self.volatility_threshold:
            ...
            return {
                ...
                'metrics': {
                    ...
                    'baseline_volatility': float(self.baseline_stats['volatility_mean']),
    ...
    def _detect_price_gap_anomaly(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        current_gap = (latest_data['open'].iloc[-1] - latest_data['close'].iloc[-2]) / \
        ...
        gap_z_score = (current_gap - self.baseline_stats['gap_mean']) / \
    ...
    def _detect_volume_anomaly(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        if 'volume_std' not in self.baseline_stats or self.baseline_stats['volume_std'] == 0:
            return None
        
        # Z-score for volume
        volume_z_score = (recent_volume - self.baseline_stats['volume_mean']) / \
        ...
        if volume_z_score > 2.0:  # Unusually high volume
            ...
            return {
                ...
                'metrics': {
                    ...
                    'volume_ratio': float(recent_volume / self.baseline_stats['volume_mean']),
    ...
    def _detect_price_movement_anomaly(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        if 'return_std' not in self.baseline_stats or self.baseline_stats['return_std'] == 0:
            return None
        ...
        if consecutive_moves >= 3 or max_z_score > 3.0:
            anomaly_level = 'high' if (consecutive_moves >= 4 or max_z_score > 4.0) else 'medium'
            ...
            return {
                ...
                'metrics': {
                    ...
                    'recent_returns': [float(r) for r in recent_returns if not np.isnan(r)]
                },
                'description': f"Abnormal price movement pattern detected",
                'impact': 'Unusual price patterns may indicate model assumptions are invalid'
            }
        
        return None
    
    def _detect_trend_reversal_anomaly(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        if (trend_20d * trend_5d < 0 and 
            abs(trend_5d) > abs(trend_20d) * 1.5):
    ...
    def _detect_market_structure_anomaly(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_hurst_exponent(self, returns: np.ndarray, max_lag: int = 10) -> float:
    ...
    def _determine_gate_action(self, overall_level: str, anomalies: List[Dict[str, Any]]) -> str:
        ...
        if overall_level == 'critical':
            ...
            model_affecting_types = ['volatility_spike', 'trend_reversal', 'market_structure_trending']
            has_model_affecting = any(a['type'] in model_affecting_types for a in anomalies)
    ...
    def _get_current_metrics(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            '5d_volatility': float(recent_data['close'].pct_change().std() * np.sqrt(252)),
            '5d_return': float((recent_data['close'].iloc[-1] / recent_data['close'].iloc[0] - 1)),
        }
        
        if self.baseline_stats:
            metrics.update({
                'price_vs_baseline': float(metrics['current_price'] / self.baseline_stats['price_mean']),
                'volume_vs_baseline': float(metrics['current_volume'] / self.baseline_stats['volume_mean']),
                'volatility_vs_baseline': float(metrics['5d_volatility'] / self.baseline_stats['volatility_mean'])
            })
