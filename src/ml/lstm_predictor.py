...
class LSTMStockPredictor:
    ...
    def __init__(self, config: Optional[LSTMConfig] = None):
        ...
        try:
            ...
            logger.warning(f"Failed to set TensorFlow logger level: {e}")
            pass
    ...
    def prepare_data(
        self, stock_code: str, days: int = 1000
        ...
        with get_session_scope() as session:
            ...
            if len(price_history) < self.config.sequence_length + 50:
                raise ValueError(
                    f"Insufficient data for {stock_code}. Need at least {self.config.sequence_length + 50} days."
                )
    ...
    def train_model(
        ...
        logger.info(
            f"LSTM model trained for {stock_code}: Accuracy={directional_accuracy:.1%}, MAE={mae:.2f}"
        )
    ...
    def predict(self, stock_code: str, days_ahead: int = 1) -> LSTMPredictionResult:
        ...
        with get_session_scope() as session:
            recent_data = (
                ...
                .filter(PriceHistory.stock_code == stock_code)
                .order_by(PriceHistory.date.desc())
                .limit(self.config.sequence_length + 20)
                .all()
            )
            ...
            # 最新のシーケンスを取得
            latest_sequence = scaled_data[-self.config.sequence_length :].reshape(
            ...
            return LSTMPredictionResult(
                ...
                sequence_used=scaled_data[
                    -10:, 0
                ].tolist(),  # 最新10日の終値（スケール済み）
