...
class FeatureEngine:
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Drop helper columns
        df = df.drop(
            ["prev_close", "tr1", "tr2", "tr3", "true_range"], axis=1, errors="ignore"
        )
...
class StockPredictionEngine:
    ...
    async def train_model(
        ...
        try:
            ...
            metrics = self._calculate_metrics(
                y_test, y_pred, X_test.index, df.loc[X_test.index, "Close"]
            )
            ...
            logger.info(
                f"Model trained for {symbol}. R2: {metrics.r2:.3f}, Accuracy: {metrics.accuracy:.3f}"
            )
    ...
    async def predict_price(
        ...
        try:
            ...
            direction = (
                ...
                else "down" if change_percent < -0.5 else "stable"
            )
            ...
            logger.info(
                f"Prediction for {symbol}: {predicted_price:.2f} ({change_percent:+.2f}%)"
            )
    ...
    async def get_ensemble_prediction(
        self, symbol: str, horizon: PredictionHorizon = PredictionHorizon.DAILY
    ) -> Optional[PredictionResult]:
    ...
    def _calculate_metrics(self, y_true, y_pred, dates, actual_prices) -> ModelMetrics:
        ...
        predicted_directions = np.sign(
            y_pred - actual_prices.shift(1).loc[y_true.index]
        )
        ...
        try:
            predicted_returns = (
                y_pred - actual_prices.loc[y_true.index]
            ) / actual_prices.loc[y_true.index]
            sharpe_ratio = (
                np.mean(predicted_returns) / np.std(predicted_returns)
        ...
        return ModelMetrics(
            mse=mse, mae=mae, r2=r2, accuracy=accuracy, sharpe_ratio=sharpe_ratio
        )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_models": len(
                [k for k in self.models.keys() if "_" in k]
            ),  # Count symbol-specific models
        }
