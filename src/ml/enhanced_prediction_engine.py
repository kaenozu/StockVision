...
from .prediction_engine import FeatureEngine
...
class EnhancedStockPredictionEngine:
    ...
    def _optimize_hyperparameters(
        self, model_type: EnhancedModelType, X: np.ndarray, y: np.ndarray
    ) -> Any:
        ...
        try:
            ...
            logger.info(
                f"Best params for {model_type.value}: {grid_search.best_params_}"
            )
            ...
            logger.warning(
                f"Hyperparameter optimization failed for {model_type.value}: {e}"
            )
            return self.models[model_type]

    def _calculate_enhanced_metrics(
        self, y_true: np.ndarray, y_pred: np.ndarray, dates: pd.Index, prices: pd.Series
        ...
        # Financial metrics
        returns = (y_pred - prices.shift(1).loc[dates].values) / prices.shift(1).loc[
            dates
        ].values
        ...
        if len(returns) > 0:
            ...
            information_ratio = (
                np.mean(excess_returns) / tracking_error if tracking_error > 0 else 0
            )
        else:
            avg_return = volatility = sharpe_ratio = max_drawdown = hit_rate = (
                information_ratio
            ) = None
    ...
    async def train_enhanced_model(
        ...
        try:
            ...
            # Feature selection
            X_selected, selected_features = self.feature_engine.select_features(
                X, y, k=50
            )
            ...
            if model_type in [
                EnhancedModelType.ENSEMBLE_VOTING,
                EnhancedModelType.ENSEMBLE_BAGGING,
            ]:
            ...
            logger.info(
                f"Enhanced model trained for {symbol}. R2: {metrics.r2:.3f}, "
        ...
        try:
            ...
            if model_key not in self.models:
                ...
                await self.train_enhanced_model(
                    symbol, model_type, period, optimize_params
                )
