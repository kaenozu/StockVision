...
class LSTMGRUPredictionEngine:
    ...
    def create_lstm_model(
        self, symbol: str, config: Optional[LSTMConfig] = None
    ) -> LSTMModel:
    ...
    def create_gru_model(
        self, symbol: str, config: Optional[GRUConfig] = None
    ) -> GRUModel:
    ...
    def train_gru_model(
        self, symbol: str, data: np.ndarray, config: Optional[GRUConfig] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for epoch in range(model_config.num_epochs):
                ...
                if (epoch + 1) % 20 == 0:
                    logger.info(
                        f"GRU {symbol} - Epoch [{epoch+1}/{model_config.num_epochs}], Loss: {avg_loss:.6f}"
                    )
            ...
            logger.info(
                f"GRU model trained for {symbol}. Final MSE: {mse:.6f}, MAE: {mae:.6f}"
            )
    ...
    def predict_with_lstm(
        self, symbol: str, data: np.ndarray, steps: int = 1
    ) -> np.ndarray:
        ...
        try:
            ...
            input_tensor = (
                torch.FloatTensor(input_sequence)
                .unsqueeze(0)
                .unsqueeze(-1)
                .to(self.device)
            )
