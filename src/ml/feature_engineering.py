...
class FeatureEngineer:
    ...
    def _add_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["vwap"] = (df["typical_price"] * df["volume"]).rolling(window=20).sum() / df[
            "volume"
    ...
    def _add_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["days_since_last_friday"] = df.apply(
            lambda row: (
                (row["date"].weekday() + 3) % 7 if row["date"].weekday() < 5 else 0
            ),
            axis=1,
        )
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["volatility_10d"] = df["price_change"].rolling(window=10).std() * np.sqrt(
            252
        )
        df["volatility_20d"] = df["price_change"].rolling(window=20).std() * np.sqrt(
            252
        )
        ...
        df["volatility_regime"] = (
            df["volatility_20d"] > df["volatility_20d"].rolling(window=60).mean()
        ).astype(int)
    ...
    def validate_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        validation_results["quality_score"] = (
            1.0 - (total_missing + total_infinite) / total_cells
        )
    ...
    def _add_simple_indicators(self, df: pd.DataFrame):
        ...
        df["rsi_30"] = (
            df["close"]
            .rolling(window=30)
            .apply(lambda x: self._simple_rsi(x), raw=False)
        )
