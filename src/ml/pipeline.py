...
from .backtesting import BacktestConfig, BacktestingEngine, BacktestResult
from .feature_engineering import FeatureEngineer as AdvancedFeatureEngine
from .lstm_gru_models import GRUConfig, LSTMConfig, LSTMGRUPredictionEngine
from .prediction_engine import ModelType, StockPredictionEngine
...
class MLPipeline:
    ...
    async def run_pipeline(self, symbol: str) -> PipelineResult:
        ...
        try:
            ...
            best_model, ensemble_weights = await self._select_best_models(
                training_results
            )
            ...
            logger.info(
                f"ML pipeline completed for {symbol} in {total_time:.2f} seconds"
            )
    ...
    async def _train_ml_models(
        ...
        X = features.drop(
            columns=[
                col for col in features.columns if "target" in col or col == "date"
            ]
        )
        ...
        for model_type in model_types:
            try:
                ...
                # Train model
                metrics = await self.prediction_engine.train_model(
                    symbol, model_type, self.config.data_period
                )
                ...
                if metrics:
                    ...
                    logger.info(
                        f"Trained {model_type.value} for {symbol} (R2: {metrics.r2:.3f})"
                    )
    ...
    async def _train_dl_models(
        ...
        train_size = int(
            len(close_prices)
            * (1 - self.config.test_size - self.config.validation_size)
        )
        val_size = int(len(close_prices) * self.config.validation_size)

        train_data = close_prices[:train_size]
        # val_data = close_prices[train_size:train_size + val_size] # Commented out as it's unused
        ...
        try:
            ...
            lstm_metrics = self.lstm_gru_engine.train_lstm_model(
                symbol, train_data, lstm_config
            )

            # Make predictions
            predictions = self.lstm_gru_engine.predict_with_lstm(
                symbol, train_data, len(test_data)
            )
        ...
        try:
            ...
            gru_metrics = self.lstm_gru_engine.train_gru_model(
                symbol, train_data, gru_config
            )

            # Make predictions
            predictions = self.lstm_gru_engine.predict_with_gru(
                symbol, train_data, len(test_data)
            )
    ...
    async def _select_best_models(
        self, results: List[ModelTrainingResult]
        ...
        sorted_results = sorted(
            results, key=lambda x: x.metrics.get("r2", 0), reverse=True
        )
    ...
    async def _run_backtesting(
        self, symbol: str, data: pd.DataFrame, results: List[ModelTrainingResult]
        ...
        for result in results:
            if result.predictions is not None:
                ...
                pred_series = pd.Series(
                    result.predictions, index=data.index[-len(result.predictions) :]
                )
