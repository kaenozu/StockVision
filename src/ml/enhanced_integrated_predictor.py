...
class EnhancedIntegratedPredictor:
    ...
    def enhance_price_data(
        ...
        # 高度な特徴量を追加
        try:
            enhanced_df = self.feature_engine.create_all_advanced_features(
                df, stock_code
            )
            logger.info(
                f"Enhanced {stock_code} data with {enhanced_df.shape[1]} features"
            )
            return enhanced_df
    ...
    def calculate_feature_importance(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        technical_features = [
            ...
            if any(
                x in col.lower()
                for x in ["sma", "ema", "rsi", "macd", "bb_", "returns"]
            )
        ]
    ...
    def analyze_market_context(self, stock_code: str) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                ...
                "regime_factor": (
                    0.1
                    if market_ctx.regime.value in ["bull_market", "low_volatility"]
                    else -0.1
                ),
    ...
    def predict_enhanced(
        self, stock_code: str, use_advanced_features: bool = True
        ...
        try:
            ...
            if use_advanced_features:
                ...
                if enhanced_df is None:
                    logger.warning(
                        f"Failed to enhance data for {stock_code}, using base prediction"
                    )
            ...
            if use_advanced_features:
                ...
                # current_price = base_result.technical_indicators['current_price'] # Commented out as it's unused
                ...
                return EnhancedPredictionResult(
                    ...
                    prediction_components={
                        "base_prediction": base_result.predicted_price
                    },
                    feature_importance={},
                    prediction_date=datetime.now(),
                )
    ...
    def get_prediction_explanation(
        self, result: EnhancedPredictionResult
    ) -> Dict[str, Any]:
        """予想結果の説明を生成"""
        current_price = result.prediction_components.get(
            "base_prediction", result.predicted_price
        )
        ...
        explanation = {
            "summary": {
                "direction": (
                    "上昇" if change > 0 else "下降" if change < 0 else "横ばい"
                ),
                "magnitude": abs(change_percent),
                "confidence_level": (
                    "高"
                    if result.confidence > 0.7
                    else "中" if result.confidence > 0.5 else "低"
                ),
            },
            "key_factors": {
                "market_context": (
                    "好材料"
                    if result.market_context_score > 0
                    else "悪材料" if result.market_context_score < 0 else "中立"
                ),
                "seasonal_effect": (
                    "追い風"
                    if result.seasonal_factor > 0
                    else "逆風" if result.seasonal_factor < 0 else "影響なし"
                ),
            ...
            "model_components": {
                ...
                "ensemble_weight": result.prediction_components.get(
                    "ensemble_component", 0
                ),
                "adjustments": {
                    "market": result.prediction_components.get("market_adjustment", 0),
                    "seasonal": result.prediction_components.get(
