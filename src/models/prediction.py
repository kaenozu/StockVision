...
class PredictionModel(Base):
    ...
    feature_names: Mapped[Dict[str, Any]] = mapped_column(
    ...
    last_trained: Mapped[datetime] = mapped_column(
        DateTime, default=datetime.utcnow, nullable=False
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "model_type IN ('short_term', 'medium_term', 'long_term')",
            name="check_model_type",
        ),
        CheckConstraint(
            "algorithm IN ('random_forest', 'linear_regression', 'svr')",
...
class PredictionResult(Base):
    ...
    error_percentage: Mapped[Optional[float]] = mapped_column(
        DECIMAL(7, 4), nullable=True
    )

    # Feature snapshot
    individual_predictions: Mapped[Dict[str, Any]] = mapped_column(
        MutableDict.as_mutable(JSON), nullable=False
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "predicted_action IN ('buy', 'sell', 'hold')", name="check_predicted_action"
        ),
        CheckConstraint(
            "actual_action IS NULL OR actual_action IN ('buy', 'sell', 'hold')",
            name="check_actual_action",
        ),
...
class TrainingDataset(Base):
    ...
    # Dataset structure
    feature_columns: Mapped[Dict[str, Any]] = mapped_column(
        MutableDict.as_mutable(JSON), nullable=False
    )
    target_columns: Mapped[Dict[str, Any]] = mapped_column(
        MutableDict.as_mutable(JSON), nullable=False
    )
    ...
    def validate_quality_score(self, key, quality_score):
        ...
        if self.total_records and self.missing_records is not None:
            expected_quality = (
                self.total_records - self.missing_records
            ) / self.total_records
            if (
                abs(quality_score - expected_quality) > 0.001
            ):  # Allow small floating point differences
                raise ValueError(
                    "Quality score must equal (total_records - missing_records) / total_records"
                )
...
class AccuracyHistory(Base):
    ...
    # Detailed metrics
    mae_score: Mapped[Optional[float]] = mapped_column(
        DECIMAL(10, 6), nullable=True
    )  # Mean Absolute Error
    mse_score: Mapped[Optional[float]] = mapped_column(
        DECIMAL(10, 6), nullable=True
    )  # Mean Squared Error
    rmse_score: Mapped[Optional[float]] = mapped_column(
        DECIMAL(10, 6), nullable=True
    ...
    __table_args__ = (
        CheckConstraint(
            "total_predictions >= correct_predictions", name="check_prediction_counts"
        ),
        CheckConstraint(
            "accuracy_percentage >= 0.0 AND accuracy_percentage <= 100.0",
            name="check_accuracy_percentage",
        ),
    ...
    def validate_accuracy_percentage(self, key, accuracy_percentage):
        ...
        if self.total_predictions and self.correct_predictions is not None:
            ...
            if (
                abs(accuracy_percentage - expected_accuracy) > 0.01
            ):  # Allow small floating point differences
                raise ValueError(
                    "Accuracy percentage must equal (correct_predictions / total_predictions) * 100"
                )
...
class MarketAnomalyDetection(Base):
    ...
    stock_code: Mapped[Optional[str]] = mapped_column(
        String(4), ForeignKey("stocks.stock_code"), nullable=True
    )  # Null for market-wide anomalies
    ...
    # Anomaly details
    detected_metrics: Mapped[Dict[str, Any]] = mapped_column(
        MutableDict.as_mutable(JSON), nullable=False
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "anomaly_level IN ('low', 'medium', 'high', 'critical')",
            name="check_anomaly_level",
        ),
        CheckConstraint(
            "prediction_gate_action IN ('warning', 'suspend', 'block')",
            name="check_gate_action",
        ),
        CheckConstraint(
            "affected_period_start <= affected_period_end OR affected_period_end IS NULL",
