...
class WatchlistCreateRequest(BaseModel):
    ...
    alert_price_high: Optional[Decimal] = Field(
        None, gt=0, description="高値アラート価格"
    )
    alert_price_low: Optional[Decimal] = Field(
        None, gt=0, description="安値アラート価格"
    )
...
async def add_to_watchlist(
    ...
    try:
        ...
        existing_item = (
            db.query(Watchlist)
            .filter(Watchlist.stock_code == request.stock_code, Watchlist.is_active)
            .first()
        )

        if existing_item:
            logger.warning(
                f"Stock {request.stock_code} already exists in active watchlist"
            )
            raise HTTPException(
                status_code=409,
        ...
        # アラート価格の関係性チェック
        if (
            new_item.alert_price_high is not None
            and new_item.alert_price_low is not None
            and new_item.alert_price_high < new_item.alert_price_low
        ):
            raise HTTPException(
                status_code=400,
        ...
        logger.info(
            f"Successfully added stock {request.stock_code} to watchlist with ID {new_item.id}"
        )
...
async def remove_from_watchlist(
    ...
    try:
        ...
        if watchlist_item is None:
            ...
            raise HTTPException(
                status_code=404, detail=f"Watchlist item with ID {id} not found"
            )
