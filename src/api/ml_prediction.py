...
from ..ml.enhanced_prediction_engine import enhanced_prediction_engine, EnhancedModelType, EnhancedModelMetrics
...
from ..services.backtester import PredictionBacktester, BacktestResult
import logging
...
try:
    from ..ml.lstm_predictor import lstm_predictor, TENSORFLOW_AVAILABLE
...
class BacktestRequest(BaseModel):
    """バックテストリクエストモデル"""
    stock_codes: Optional[List[str]] = Field(None, description="テスト対象銘柄のリスト（指定なしで全銘柄）")
    test_days: int = Field(30, description="テスト期間（日数）", ge=10, le=100)
    prediction_horizon: int = Field(1, description="予想対象日（1=翌日、7=1週間後）", ge=1, le=30)
...
class LSTMTrainRequest(BaseModel):
    """LSTM訓練リクエスト"""
    stock_codes: Optional[List[str]] = Field(None, description="訓練対象銘柄（指定なしで全銘柄）")
...
async def get_enhanced_ml_prediction(
    ...
    try:
        logger.info(f"Enhanced ML prediction request for {stock_code}, model: {model_type}")
        ...
        try:
            ...
            df_features = enhanced_prediction_engine.feature_engine.create_advanced_features(df)
            ...
            direction = "up" if predicted_return > 0.01 else "down" if predicted_return < -0.01 else "stable"
            confidence = min(max(metrics.accuracy, 0.5), 1.0)  # Ensure reasonable confidence range
        ...
        if predicted_return > 0.03 and confidence > 0.7:
            action = "buy"
            reasoning = f"高信頼度({confidence:.1%})で{predicted_return:.2%}の上昇を予測"
        elif predicted_return < -0.03 and confidence > 0.7:
            action = "sell" 
            reasoning = f"高信頼度({confidence:.1%})で{predicted_return:.2%}の下落を予測"
        else:
            action = "hold"
            reasoning = f"中程度の変動予測({predicted_return:.2%})、信頼度{confidence:.1%}"
...
async def run_prediction_backtest(request: BacktestRequest):
    ...
    try:
        logger.info(f"バックテスト開始: 銘柄数={len(request.stock_codes) if request.stock_codes else 'ALL'}, "
        ...
        if not backtest_results:
            raise HTTPException(status_code=404, detail="バックテスト結果が取得できませんでした")
        
        # レスポンス用のデータを構築
        stock_results = []
        for stock_code, result in backtest_results.items():
            stock_results.append(BacktestResponseItem(
        ...
        overall_metrics = {
            ...
            "successful_stocks": len([r for r in backtest_results.values() if r.total_predictions > 0]),
            "total_predictions": sum(r.total_predictions for r in backtest_results.values()),
            "average_directional_accuracy": summary_report.get("average_directional_accuracy", 0),
            ...
            "best_performers": {
                "by_accuracy": summary_report.get("best_stock", {}).get("by_directional", ["N/A", None])[0] if summary_report.get("best_stock") else "N/A",
                "by_rmse": summary_report.get("best_stock", {}).get("by_rmse", ["N/A", None])[0] if summary_report.get("best_stock") else "N/A"
            },
            "worst_performers": {
                "by_accuracy": summary_report.get("worst_stock", {}).get("by_directional", ["N/A", None])[0] if summary_report.get("worst_stock") else "N/A",
                "by_rmse": summary_report.get("worst_stock", {}).get("by_rmse", ["N/A", None])[0] if summary_report.get("worst_stock") else "N/A"
            }
        }
        ...
        test_summary = {
            ...
            "data_quality": {
                ...
                "average_predictions_per_stock": sum(r.total_predictions for r in backtest_results.values()) / len(backtest_results) if backtest_results else 0
            }
        }
        ...
        logger.info(f"バックテスト完了: ID={request_id}, "
        ...
        raise HTTPException(status_code=500, detail=f"バックテスト処理中にエラーが発生しました: {str(e)}")
...
async def run_single_stock_backtest(
    ...
    try:
        ...
        logger.info(f"単一銘柄バックテスト完了: {stock_code}, "
...
async def get_lstm_prediction(
    ...
    try:
        logger.info(f"LSTM prediction request for {stock_code}, days_ahead: {days_ahead}")
        ...
        logger.info(f"LSTM prediction completed for {stock_code}: "
...
async def train_lstm_models(request: LSTMTrainRequest):
    ...
    try:
        ...
        logger.info(f"LSTM training request: {request.stock_codes}, force_retrain={request.force_retrain}")
        ...
        for stock_code in target_stocks:
            try:
                ...
                metrics = lstm_predictor.train_model(stock_code, force_retrain=request.force_retrain)
        ...
        logger.info(f"LSTM training completed: {successful_trainings}/{len(target_stocks)} models trained")
        return response
...
async def get_lstm_status():
    ...
    try:
        ...
        with get_session_scope() as db:
            ...
            available_stocks = [stock.stock_code for stock in stocks[:20]]  # 最大20銘柄表示
