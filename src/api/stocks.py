...
from ..stock_api.data_models import CurrentPriceResponse, StockData
from ..stock_storage.database import get_session_scope
from ..utils.cache import cache_current_price, cache_stock_data
...
async def get_stock_info(
    ...
    try:
        # バリデーション (StockCode(code=stock_code) is used implicitly by Pydantic Path validation)
        # stock_validator = StockCode(code=stock_code) # Commented out as it's unused

        # ハイブリッドサービスを使用
        stock_service = await get_stock_service()
        stock_data = await stock_service.get_stock_info(
            stock_code=stock_code, use_real_data=use_real_data, db=db
        )
...
async def get_current_price(
    ...
    """
    logger.info(
        f"Fetching current price for {stock_code} (use_real_data={use_real_data})"
    )
    ...
    try:
        ...
        current_price = await stock_service.get_current_price(
            stock_code=stock_code, use_real_data=use_real_data
        )
...
async def get_price_history(
    ...
    logger.info(
        f"Fetching price history for {stock_code}, days={days} (use_real_data={use_real_data})"
    )
    ...
    try:
        ...
        price_history_data = await stock_service.get_price_history(
            stock_code=stock_code, days=days, use_real_data=use_real_data, db=db
        )
        ...
        logger.info(
            f"Successfully retrieved {len(history)} price history records for {stock_code}"
        )
        return history
