...
class StockVisionException(Exception):
    ...
    def __init__(
        self,
        message: str,
        error_code: str = "STOCK_VISION_ERROR",
...
async def stock_vision_exception_handler(
    request: Request, exc: StockVisionException
) -> JSONResponse:
...
async def stock_not_found_handler(
    request: Request, exc: StockNotFoundError
) -> JSONResponse:
