...
async def get_price_prediction_chart(
    symbol: str,
    period: str = Query(
        "short", description="Prediction period: short (7 days) or medium (14 days)"
    ),
) -> JSONResponse:
    """
    ...
    try:
            ...
            with get_session_scope() as session:
                ...
                if stock:
                    ...
                    recent_prices = (
                        ...
                        .filter(PriceHistory.stock_code == symbol)
                    ...
                    if recent_prices:
                        ...
                        logger.info(
                            f"Found current price for {symbol} from price_history: {current_price}"
                        )
                    else:
        ...
        while current_date <= end_date:
            if True:  # Include all days including weekends
                ...
                if current_date == end_date or days_count == total_days:
                    ...
                    daily_variation = (
                        (symbol_seed * days_count) % 201 - 100
                    ) / 10000  # ±1%
        ...
        chart_data = {
            ...
            "chartData": {"labels": all_dates, "datasets": datasets},
            "markers": {"buy": [], "sell": []},
            "chartType": "prediction",
            "generatedAt": datetime.now().isoformat(),
        }
    ...
    try:
        ...
        with get_session_scope() as session:
            ...
            if len(recent_history) >= 10:
                ...
                numerator = sum(
                    (x_values[i] - x_mean) * (prices[i] - y_mean) for i in range(n)
                )
                ...
                momentum_decay = (
                    sum(momentum_periods) / len(momentum_periods)
                    if momentum_periods
                    else 0.8
                )
                ...
                volume_volatility = 0
                if len(volumes) >= 5:
                    vol_variance = sum((v - avg_volume) ** 2 for v in volumes) / len(
                        volumes
                    )
...
def generate_ml_prediction(
    ...
    if len(prices) >= 3:
        # short_trend = (prices[-1] - sma_3) / sma_3 # Commented out as it's unused
        pass
    else:
        pass  # short_trend = 0 # Commented out as it's unused
    ...
    volumes = [
        float(record["volume"]) for record in historical_data if "volume" in record
    ]
    if len(volumes) >= 3:
    ...
    # volatility_factor = volatility * day * 0.1 # Commented out as it's unused
