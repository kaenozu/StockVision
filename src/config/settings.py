...
class YahooFinanceConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Enable real Yahoo Finance API calls"
    )
    max_requests: int = Field(
        default=10, description="Maximum requests per time window"
    )
    ...
    cache_ttl: int = Field(
        default=300, description="Cache TTL for API responses in seconds"
    )
...
class CacheConfig(BaseModel):
    ...
    stock_info_ttl: float = Field(
        default=300.0, description="Stock info cache TTL in seconds"
    )
    current_price_ttl: float = Field(
        default=60.0, description="Current price cache TTL in seconds"
    )
    price_history_ttl: float = Field(
        default=600.0, description="Price history cache TTL in seconds"
    )
...
class MiddlewareConfig(BaseModel):
    ...
    cache_control_enabled: bool = Field(
        default=True, description="Enable Cache Control Middleware"
    )

    # Response Compression Middleware
    response_compression_enabled: bool = Field(
        default=True, description="Enable Response Compression Middleware"
    )
    response_compression_min_size: int = Field(
        default=1024, description="Minimum response size to compress (bytes)"
    )
    response_compression_gzip_level: int = Field(
        default=6, description="GZip compression level (1-9, 9 is highest compression)"
    )
    response_compression_brotli_quality: int = Field(
        default=4,
        description="Brotli compression quality (0-11, 11 is highest compression)",
    )

    # Performance Metrics Middleware
    performance_metrics_enabled: bool = Field(
        default=True, description="Enable Performance Metrics Middleware"
    )
...
class AppConfig(BaseModel):
    ...
    server_url: Optional[str] = Field(
        default=None, description="Explicitly configured server URL"
    )
    ...
    def from_env(cls) -> "AppConfig":
        ...
        return cls(
            ...
            middleware=MiddlewareConfig(
                cache_control_enabled=os.getenv(
                    "MIDDLEWARE_CACHE_CONTROL_ENABLED", "true"
                ).lower()
                == "true",
                response_compression_enabled=os.getenv(
                    "MIDDLEWARE_RESPONSE_COMPRESSION_ENABLED", "true"
                ).lower()
                == "true",
                response_compression_min_size=int(
                    os.getenv("MIDDLEWARE_RESPONSE_COMPRESSION_MIN_SIZE", "1024")
                ),
                response_compression_gzip_level=int(
                    os.getenv("MIDDLEWARE_RESPONSE_COMPRESSION_GZIP_LEVEL", "6")
                ),
                response_compression_brotli_quality=int(
                    os.getenv("MIDDLEWARE_RESPONSE_COMPRESSION_BROTLI_QUALITY", "4")
                ),
                performance_metrics_enabled=os.getenv(
                    "MIDDLEWARE_PERFORMANCE_METRICS_ENABLED", "true"
                ).lower()
                == "true",
            ),
            cors=CorsConfig(
