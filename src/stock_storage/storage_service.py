...
class StockStorageService:
    ...
    def create_stock(
        ...
        try:
            with self._get_session_context() as session:
                # 重複チェック
                existing_stock = (
                    session.query(Stock).filter_by(stock_code=stock_code).first()
                )
                if existing_stock:
    ...
    def add_price_history(
        ...
        try:
            ...
            raise StorageError(
                f"Price history for {stock_code} on {date} already exists"
            ) from e
        except SQLAlchemyError as e:
    ...
    def get_price_history(
        ...
        try:
            with self._get_session_context() as session:
                ...
                logger.info(
                    f"Retrieved {len(price_history)} price history records for stock {stock_code}"
                )
                return price_history
    ...
    def get_latest_price_history(self, stock_code: str) -> Optional[PriceHistory]:
        ...
        try:
            ...
            logger.error(
                f"Database error getting latest price history for {stock_code}: {e}"
            )
            raise StorageError(
                f"Failed to get latest price history for {stock_code}"
            ) from e
