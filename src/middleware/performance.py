...
class CacheControlMiddleware(BaseHTTPMiddleware):
    ...
    CACHE_SETTINGS = {
        ...
        API_RECOMMENDED_STOCKS: {
            "max_age": CacheTTL.RECOMMENDED_STOCKS,
            "stale_while_revalidate": SWRTime.RECOMMENDED_STOCKS,
        },
        API_TRADING_RECOMMENDATIONS: {
            "max_age": CacheTTL.TRADING_RECOMMENDATIONS,
            "stale_while_revalidate": SWRTime.TRADING_RECOMMENDATIONS,
        },
        API_PRICE_PREDICTIONS: {
            "max_age": CacheTTL.PRICE_PREDICTIONS,
            "stale_while_revalidate": SWRTime.PRICE_PREDICTIONS,
        },
        # Real-time data - shorter cache
        API_STOCKS_CURRENT: {
            "max_age": CacheTTL.STOCK_DATA_SHORT,
            "stale_while_revalidate": SWRTime.STOCK_DATA_SHORT,
        },
        API_STOCKS_HISTORY: {
            "max_age": CacheTTL.STOCK_HISTORY,
            "stale_while_revalidate": SWRTime.STOCK_HISTORY,
        },
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not get_middleware_config().cache_control_enabled:
...
class ResponseCompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        # Check if middleware is enabled
        if (
            not get_middleware_config().response_compression_enabled
        ):  # Directly use the config
            return await call_next(request)
        ...
        if algorithm != CompressionAlgorithm.NONE:
            ...
            if compressed_body:
                ...
                response.headers["X-Compression-Time-ms"] = str(
                    round(compression_time, 2)
                )
    ...
    def _choose_compression_algorithm(
    ...
    def _compress_response(
        self, body: bytes, algorithm: CompressionAlgorithm
    ) -> tuple[Optional[bytes], CompressionMetrics]:
        """Compress response body using the specified algorithm."""
        if not body:
            return None, CompressionMetrics(algorithm=algorithm.value)

        metrics = CompressionMetrics(algorithm=algorithm.value, original_size=len(body))
        ...
        try:
            ...
            metrics.compression_ratio = metrics.compressed_size / max(
                1, metrics.original_size
            )
            ...
            return body, CompressionMetrics(
                algorithm="none", original_size=len(body), compressed_size=len(body)
            )
...
class PerformanceMetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        # Check if middleware is enabled
        if (
            not get_middleware_config().performance_metrics_enabled
        ):  # Directly use the config
            return await call_next(request)
        ...
        response.headers["X-Process-Time"] = str(
            round(process_time * TimeConstants.MILLISECONDS_PER_SECOND, 2)
        )  # milliseconds
...
def compress_price_history(history_data: list) -> dict:
    ...
    return {"dates": dates, "prices": prices, "volumes": volumes, "count": len(dates)}
