    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        logger.info(
            ...
            extra={
                ...
                "body": (
                    body[:500] if body and self.debug else None
                ),  # Only in debug mode
            ...
            return JSONResponse(
                ...
                content={
                    "error": {
                        ...
                        "details": (
                            exc.errors() if self.debug else "Request validation failed"
                        ),
                        "request_id": request_id,
                        "timestamp": datetime.utcnow().isoformat(),
                        "path": request.url.path,
            ...
            return JSONResponse(
                ...
                content={
                    "error": {
                        "code": 500,
                        "message": (
                            "Internal server error" if not self.debug else str(exc)
                        ),
...
class StockNotFoundError(HTTPException):
    ...
    def __init__(self, stock_code: str):
        super().__init__(status_code=404, detail=f"Stock code {stock_code} not found")
...
class WatchlistItemNotFoundError(HTTPException):
    ...
    def __init__(self, item_id: int):
        super().__init__(
            status_code=404, detail=f"Watchlist item with ID {item_id} not found"
        )
...
class ExternalApiError(HTTPException):
    ...
    def __init__(self, service: str = "External API", detail: str = None):
        super().__init__(
            status_code=503, detail=detail or f"{service} is temporarily unavailable"
        )
