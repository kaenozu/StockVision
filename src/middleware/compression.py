...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if not any(encoding in accept_encoding for encoding in ['br', 'gzip', 'deflate']):
            return await call_next(request)
        ...
        algorithm = self._select_compression_algorithm(accept_encoding, content_type, content_length)
        if not algorithm:
            return response
        ...
        try:
            if hasattr(self, '_get_compression_level'):
                ...
                compressed_body, encoding = await self._compress_content(body, algorithm, content_type)
            else:
                # Base CompressionMiddleware
                compressed_body, encoding = await self._compress_content(body, algorithm)
            ...
            compressed_response.headers['x-compression-ratio'] = f"{len(compressed_body) / content_length:.3f}"
    ...
    def _select_compression_algorithm(self, accept_encoding: str, content_type: str, content_length: int) -> Optional[str]:
        ...
        if 'br' in encodings and content_type.startswith(('text/', 'application/json', 'application/javascript')):
    ...
    async def _compress_content(self, content: bytes, algorithm: str) -> Tuple[bytes, str]:
        ...
        if algorithm == 'br':
            ...
            compressed = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: gzip.compress(content, compresslevel=self.compression_levels['gzip'])
            )
            ...
            compressed = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: zlib.compress(content, level=self.compression_levels['deflate'])
            )
...
class SmartCompressionMiddleware(CompressionMiddleware):
    ...
    def _get_compression_level(self, algorithm: str, content_length: int, content_type: str) -> int:
    ...
    async def _compress_content(self, content: bytes, algorithm: str, content_type: str = '') -> Tuple[bytes, str]:
        ...
        if algorithm == 'br':
            compressed = await asyncio.get_event_loop().run_in_executor(
                None, 
                lambda: brotli.compress(content, quality=level)
            )
            return compressed, 'br'
            ...
            compressed = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: gzip.compress(content, compresslevel=level)
            )
            return compressed, 'gzip'
            ...
            compressed = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: zlib.compress(content, level=level)
            )
            return compressed, 'deflate'
