...
class SmartCacheStrategy:
    ...
    def get_cache_key(self, request: Request) -> str:
        ...
        return (
            f"stockvision:cache:{hashlib.sha256(key_string.encode()).hexdigest()[:16]}"
        )
    ...
    def calculate_optimal_ttl(self, endpoint_key: str, request: Request) -> int:
        ...
        time_since_last_access = (
            (datetime.now() - metrics.last_access).total_seconds()
        ...
        recency_factor = max(
            0.5, 1 - (time_since_last_access / 3600)
        )  # Decay over 1 hour
        ...
        optimal_ttl = max(
            base_config["min_ttl"], min(optimal_ttl, base_config["max_ttl"])
        )
    ...
    def update_metrics(
        ...
        if response_size > 0:
            ...
            total_size = (
                metrics.avg_response_size * (metrics.request_count - 1) + response_size
            )
        ...
        self.global_stats.hit_rate = self.global_stats.hits / max(
            self.global_stats.total_requests, 1
        )
    ...
    def get_cache_headers(self, ttl: int, hit: bool = False) -> Dict[str, str]:
        ...
        if ttl > 0:
            headers["Cache-Control"] = (
                f"max-age={ttl}, stale-while-revalidate={ttl // 2}"
            )
...
class SmartCacheMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if cached_response:
            ...
            self.strategy.update_metrics(
                endpoint_key, hit=True, response_time=response_time
            )
        ...
        try:
            ...
            if self.enable_redis and self.redis_client:
                ...
                if len(self.local_cache) > 1000:  # Limit local cache size
                    ...
                    oldest_keys = sorted(self.cache_times.items(), key=lambda x: x[1])[
                        :100
                    ]
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        for key, metrics in self.strategy.endpoint_metrics.items():
            endpoint_stats[key] = {
                ...
                "last_access": (
                    metrics.last_access.isoformat() if metrics.last_access else None
                ),
            }
