        ...
        async with self._lock:
            ...
            if len(self.requests) >= self.max_requests:
                ...
                raise RateLimitError(
                    f"Rate limit exceeded, wait {sleep_time:.2f} seconds"
                )
...
class YahooFinanceClient:
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                if attempt < self.retry_attempts:
                    ...
                    logger.error(
                        f"Request failed after {self.retry_attempts + 1} attempts: {e}"
                    )
    ...
    def _extract_stock_info(
        self, ticker_info: Dict[str, Any], stock_code: str
    ) -> StockData:
        ...
        try:
            ...
            volume = ticker_info.get("volume") or ticker_info.get(
            ...
            day_high = ticker_info.get("dayHigh") or ticker_info.get(
                "regularMarketDayHigh"
            )
            day_low = ticker_info.get("dayLow") or ticker_info.get(
                "regularMarketDayLow"
            )
            ...
            return StockData(
                ...
                dividend_yield=(
                    Decimal(str(dividend_yield * 100)) if dividend_yield else None
                ),
            ...
            raise DataValidationError(
                f"Invalid data format for stock {stock_code}"
            ) from e

    def _extract_price_history(
        self, history_data: Any, stock_code: str, days: int
    ) -> PriceHistoryData:
        ...
        try:
            if history_data.empty:
                ...
                return PriceHistoryData(
                    stock_code=stock_code, history=[], start_date=None, end_date=None
                )
            ...
            for date_index, row in history_data.iterrows():
                try:
                    ...
                    trade_date = (
                        date_index.date() if hasattr(date_index, "date") else date_index
                    )
                    ...
                    if (
                        ...
                        logger.debug(
                            f"Skipping row with NaN values for {stock_code} on {date_index}"
                        )
                        continue
                    ...
                    logger.warning(
                        f"Skipping invalid price data for {stock_code} on {date_index}: {e}"
                    )
            ...
            if not history_items:
                raise DataValidationError(
                    f"No valid price history data for stock {stock_code}"
                )
            ...
            raise DataValidationError(
                f"Failed to extract price history for stock {stock_code}"
            ) from e

    @cache_stock_data(
        cache_key_func=lambda self, stock_code: f"stock_info:{stock_code}"
    )
        ...
        try:
            ...
            if (
                ...
                raise StockNotFoundError(
                    f"Stock {stock_code} not found or no price data available"
                )
    ...
    @cache_price_history(
        cache_key_func=lambda self, stock_code, days=30: f"price_history:{stock_code}:{days}"
    )
    async def _async_get_price_history(
        ...
        try:
            ...
            history = await loop.run_in_executor(
                None, lambda: ticker.history(start=start_date, end=end_date)
            )

            price_history_data = self._extract_price_history(history, stock_code, days)
            logger.info(
                f"Successfully retrieved {len(price_history_data.history)} price history records for {stock_code}"
            )
            ...
            raise YahooFinanceError(
                f"Failed to get price history for {stock_code}"
            ) from e
    ...
    @cache_current_price(
        cache_key_func=lambda self, stock_code: f"current_price:{stock_code}"
    )
