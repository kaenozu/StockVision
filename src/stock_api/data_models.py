...
class CurrentPrice(BaseModel):
    ...
    market_cap: Optional[Decimal] = Field(
        None, gt=0, description="Market capitalization"
    )
    ...
    model_config = {"populate_by_name": True, "json_encoders": {Decimal: float}}
    ...
    def validate_price_consistency(self):
        ...
        if current is not None and previous is not None and change is not None:
            ...
            if abs(calculated_change - change) > Decimal("0.01"):
                raise ValueError(
                    "Price change does not match current and previous prices"
                )
...
class StockData(BaseModel):
    ...
    dividend_yield: Optional[Decimal] = Field(
        None, ge=0, description="Dividend yield percentage"
    )

    # Timestamps
    last_updated: Optional[datetime] = Field(None, description="Last update timestamp")
    market_time: Optional[Union[str, int]] = Field(
        None, description="Market session time"
    )
...
class PriceHistoryData(BaseModel):
    ...
    stock_code: str = Field(..., description="4-digit stock code")
    history: List[PriceHistoryItem] = Field(
        ..., description="List of price history items"
    )
            ...
            if stock_code:
                for item in v:
                    if item.stock_code != stock_code:
                        raise ValueError(
                            f"History item stock code {item.stock_code} does not match {stock_code}"
                        )
        return v
    ...
    def validate_date_range(cls, values):
        ...
        if history:
            ...
            if start_date and actual_start < start_date:
                raise ValueError(
                    f"History contains dates before start_date {start_date}"
                )
    ...
    def sort_by_date(self, ascending: bool = True) -> List[PriceHistoryItem]:
        ...
        return sorted(
            self.history, key=lambda x: _to_dt(x.date).date(), reverse=not ascending
        )
    ...
    def filter_by_date_range(
        ...
        filtered_history = [
            ...
            if start_date
            <= (
                _dt(item.date.year, item.date.month, item.date.day).date()
                if isinstance(item.date, date)
                else (
                    _dt.strptime(item.date, "%Y-%m-%d").date()
...
class PriceHistoryRequest(BaseModel):
    ...
    stock_code: str = Field(..., description="4-digit stock code")
    days: int = Field(
        default=30, ge=1, le=3650, description="Number of days to retrieve"
    )
...
class APIResponse(BaseModel):
    ...
    data: Optional[Union[StockData, PriceHistoryData, Dict[str, Any]]] = Field(
...
class BulkStockInfoRequest(BaseModel):
    ...
    stock_codes: List[str] = Field(
        ..., min_items=1, max_items=100, description="List of stock codes"
    )
...
class BulkStockInfoResponse(BaseModel):
    ...
    results: List[APIResponse] = Field(
        ..., description="List of individual stock responses"
    )
    ...
    def validate_counts(cls, values):
        ...
        if actual_successful != total_successful:
            raise ValueError(
                "total_successful does not match actual successful results"
            )
