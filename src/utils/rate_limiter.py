...
class IntelligentRateLimiter:
    ...
    async def acquire(self, endpoint: str = "default") -> None:
        ...
        async with self._lock:
            ...
            if endpoint in self._429_timestamps:
                cooldown_end = (
                    self._429_timestamps[endpoint] + self.config.cooldown_after_429
                )
                if now < cooldown_end:
                    wait_time = cooldown_end - now
                    logger.warning(
                        f"In cooldown for {endpoint}, waiting {wait_time:.1f}s"
                    )
            ...
            if endpoint in self._last_requests:
                ...
                if time_since_last < min_interval:
                    ...
                    logger.debug(
                        f"Rate limiting {endpoint}: sleeping {sleep_time:.2f}s"
                    )
    ...
    def handle_response_code(self, endpoint: str, status_code: int) -> None:
        ...
        if status_code == 429:
            ...
            logger.warning(
                f"429 error on {endpoint}, switched to {self.strategy.value} strategy"
            )
