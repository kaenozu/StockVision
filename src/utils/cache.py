...
from ..constants import CacheSize, CacheTTL
...
class AdaptiveTTLCache:
    ...
    def _evict_entries(self, num_to_evict: int = 1) -> int:
        ...
        if self.size_based_eviction and self.max_memory_bytes:
            ...
            while current_size > self.max_memory_bytes and self._cache:
                # Evict largest entry
                key_to_evict = max(
                    self._cache.keys(), key=lambda k: self._cache[k].size_estimate
                )
        ...
        while len(self._cache) >= self.maxsize and evicted < num_to_evict:
            ...
            candidate_keys = list(self._cache.keys())[
                : min(10, len(self._cache))
            ]  # Sample first 10
            key_to_evict = min(
                candidate_keys,
                key=lambda k: (
    ...
    def _calculate_adaptive_ttl(self, entry: CacheEntry) -> float:
        ...
        ):  # Accessed less than once every 10 seconds on average
        ...
        with self._lock:
            ...
            if self.use_redis and self._redis_client:
                ...
                try:
                    ...
                    logger.debug(
                        f"Set value in Redis cache for {redis_key} (TTL: {self.ttl}s)"
                    )
                except Exception as e:
...
_use_redis = (
    bool(_settings.redis_host) or os.getenv("ENABLE_REDIS", "false").lower() == "true"
)
...
def cache_stock_data(
    cache_key_func: Optional[Callable] = None, ttl: float = CacheTTL.STOCK_DATA_SHORT
):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cache_key_func:
                ...
                cache_key = generate_stock_cache_key(
                    func.__name__, stock_code, **cache_kwargs
                )
            ...
            logger.debug(
                f"Cache miss, stored result for {cache_key} (size: {size_estimate} bytes)"
            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cache_key_func:
                ...
                cache_kwargs = {k: v for k, v in kwargs.items() if k != "stock_code"}
                cache_key = generate_stock_cache_key(
                    "price_history", stock_code, **cache_kwargs
                )
            ...
            size_estimate = (
                len(str(result)) * 2 if result else 0
            )  # Rough estimate for list/dict
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cache_key_func:
                ...
                cache_kwargs = {k: v for k, v in kwargs.items() if k != "stock_code"}
                cache_key = generate_stock_cache_key(
                    "current_price", stock_code, **cache_kwargs
                )
...
def invalidate_stock_cache(stock_code: str) -> None:
    ...
    logger.info(
        f"Invalidated {len(keys_to_remove)} cache entries for stock {stock_code}"
    )
