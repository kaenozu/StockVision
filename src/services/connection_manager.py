...
class ConnectionManager:
    ...
    async def create_connection(
        ...
        async with self._lock:
            ...
            connection_info = ConnectionInfo(
                id=connection_id, url=url, status=ConnectionStatus.DISCONNECTED
            )
            self.connections[connection_id] = connection_info

            # Start connection
            return await self._connect(
                connection_info, connect_callback, disconnect_callback, error_callback
            )
    ...
    async def _connect(
        ...
        try:
            ...
            logger.info(
                f"Connected to {connection_info.url} (ID: {connection_info.id})"
            )
            ...
            logger.error(
                f"Failed to connect to {connection_info.url} (ID: {connection_info.id}): {error_msg}"
            )
    ...
    async def disconnect(
        self, connection_id: str, disconnect_callback: Optional[Callable] = None
    ):
        ...
        async with self._lock:
            if connection_id in self.connections:
                ...
                logger.info(
                    f"Disconnected from {connection_info.url} (ID: {connection_id})"
                )
    ...
    async def _schedule_reconnect(
        ...
        if connection_info.reconnect_attempts >= self.config.max_reconnect_attempts:
            logger.error(
                f"Max reconnection attempts reached for {connection_info.url} (ID: {connection_info.id})"
            )
            return
        ...
        logger.info(
            f"Scheduling reconnection for {connection_info.url} (ID: {connection_info.id}) in {delay:.2f}s"
        )
    ...
    async def _reconnect_after_delay(
        ...
        try:
            ...
            logger.info(
                f"Reconnection cancelled for {connection_info.url} (ID: {connection_info.id})"
            )
        except Exception as e:
            logger.error(
                f"Error in reconnection task for {connection_info.url} (ID: {connection_info.id}): {e}"
            )
    ...
    async def _heartbeat_task(
        ...
        try:
            while (
                self.is_running and connection_info.status == ConnectionStatus.CONNECTED
            ):
                try:
                    ...
                    time_since_activity = (
                        datetime.now() - connection_info.last_activity
                    ).total_seconds()
                    if time_since_activity > self.config.heartbeat_timeout:
                        ...
                        await self._handle_connection_timeout(
                            connection_info, disconnect_callback
                        )
