...
class HybridStockService:
    ...
    async def get_stock_info(
        ...
        if should_use_real:
            ...
            if db:
                db_stock = (
                    db.query(Stock).filter(Stock.stock_code == stock_code).first()
                )
            ...
            try:
                ...
                logger.warning(
                    f"Real API failed for {stock_code}, falling back to mock data: {e}"
                )
                # Fall back to mock data
                pass
            except Exception as e:
                logger.error(
                    f"Unexpected error getting real stock info for {stock_code}: {e}"
                )
                # Fall back to mock data
                pass
        ...
        logger.info(
            f"Generating sample stock data for {stock_code} - consider using CSV import for real data"
        )
        from datetime import datetime
    ...
    async def get_current_price(
        self, stock_code: str, use_real_data: Optional[bool] = None
    ) -> CurrentPrice:
        ...
        if should_use_real:
            ...
            try:
                ...
                logger.info(
                    f"Successfully retrieved real current price for {stock_code}"
                )
                return price_data

            except Exception as e:
                logger.warning(
                    f"Real API failed for current price {stock_code}, using mock: {e}"
                )
        ...
        logger.info(
            f"Generating sample current price for {stock_code} - consider using CSV import for real data"
        )
        from datetime import datetime
        from decimal import Decimal
    ...
    async def get_price_history(
        ...
        logger.info(
            f"Getting price history for {stock_code} (use_real_data={should_use_real})"
        )
        ...
        if should_use_real:
            ...
            if db:
                ...
                if (
                    db_history and len(db_history) >= days * 0.7
                ):  # At least 70% of requested days
                    logger.info(
                        f"Using database price history for {stock_code}: {len(db_history)} records"
                    )
                    history_items = []
                    ...
                    history_data = PriceHistoryData(
                        stock_code=stock_code, history=history_items, period_days=days
                    )
                    # Also cache it
                    await self.cache.set(
                        "price_history", stock_code, history_data, days=days
                    )
                    return history_data
            ...
            try:
                ...
                # Cache the result
                await self.cache.set(
                    "price_history", stock_code, price_history_data, days=days
                )
                # Optionally save to database
                if db:
                ...
                logger.info(
                    f"Successfully retrieved real price history for {stock_code}"
                )
        ...
        cached_mock_history = await self.cache.get(
            "price_history",
            stock_code,
            ttl=self.cache_config.price_history_ttl,
            days=days,
        )
        ...
        logger.warning(
            f"Generating sample price history for {stock_code} as fallback - consider using CSV import for real data"
        )
        from datetime import datetime, timedelta
        from decimal import Decimal
    ...
    async def _save_stock_to_db(self, stock_data: StockData, db: Session) -> None:
        ...
        try:
            existing_stock = (
                db.query(Stock)
                .filter(Stock.stock_code == stock_data.stock_code)
                .first()
            )
    ...
    async def _save_price_history_to_db(
        self, history_data: PriceHistoryData, db: Session
    ) -> None:
        ...
        try:
            ...
            db.commit()
            logger.debug(
                f"Saved {len(history_data.history)} price history records to database"
            )
