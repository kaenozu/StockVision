...
from ..services.realtime_provider import RealTimeDataProvider, RealTimeDataPoint
...
class DataStreamingOptimizer:
    ...
    def __init__(self, config: StreamingOptimizationConfig):
        ...
        self.last_values: Dict[str, Dict[str, float]] = {}  # symbol -> field -> last_value
    ...
    async def _batch_data(self, channel: str, data: Any, client_id: Optional[str] = None):
    ...
    async def _send_optimized_data(self, channel: str, data: Any, client_id: Optional[str] = None):
    ...
    async def _send_message(self, message: StreamMessage, client_id: Optional[str] = None):
        ...
        try:
            ...
            if client_id:
                await websocket_manager.send_to_client(client_id, message_data, message.channel)
            else:
                await websocket_manager.broadcast_to_channel(message.channel, message_data)
    ...
    async def get_connection_stats(self, client_id: Optional[str] = None) -> Dict[str, Any]:
        """Get connection statistics"""
        if client_id:
            return self.connection_stats.get(client_id, {})
        return self.connection_stats
    
    async def apply_data_filtering(self, client_id: str, symbols: List[str]) -> List[str]:
        ...
        if len(symbols) > self.config.max_symbols_per_client:
            logger.warning(f"Client {client_id} requested {len(symbols)} symbols, limiting to {self.config.max_symbols_per_client}")
...
async def get_streaming_optimizer(config: Optional[StreamingOptimizationConfig] = None) -> DataStreamingOptimizer:
