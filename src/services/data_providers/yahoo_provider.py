...
class YahooFinanceProvider(BaseDataProvider):
    ...
    async def get_current_price(self, stock_code: str) -> CurrentPrice:
        ...
        try:
            client = await self._get_client()
            logger.info(f"Fetching real current price for {stock_code} from Yahoo Finance")
            ...
            self._record_request(False)
            logger.error(f"Unexpected error getting current price for {stock_code}: {e}")
            raise DataProviderError(f"Unexpected error: {e}") from e
    ...
    async def get_price_history(self, stock_code: str, days: int = 30) -> PriceHistoryData:
        ...
        try:
            client = await self._get_client()
            logger.info(f"Fetching real price history for {stock_code} ({days} days) from Yahoo Finance")
            ...
            self._record_request(False)
            logger.error(f"Unexpected error getting price history for {stock_code}: {e}")
            raise DataProviderError(f"Unexpected error: {e}") from e
