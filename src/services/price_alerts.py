...
class PriceAlertSystem:
        ...
        async with self._lock:
            ...
            if alert.symbol in self.symbol_alerts:
                ...
                if not self.symbol_alerts[alert.symbol]:
                    ...
                    self.realtime_provider.unsubscribe(alert.symbol, self._on_price_update)
    ...
    async def get_alerts_for_symbol(self, symbol: str) -> List[PriceAlert]:
        ...
        async with self._lock:
            ...
            return [self.alerts[alert_id] for alert_id in alert_ids if alert_id in self.alerts]
    ...
    def _check_alert_condition(self, alert: PriceAlert, data_point: RealTimeDataPoint) -> bool:
    ...
    def _get_current_value(self, condition: AlertCondition, data_point: RealTimeDataPoint) -> float:
        ...
        elif condition in [AlertCondition.PERCENTAGE_CHANGE_ABOVE, AlertCondition.PERCENTAGE_CHANGE_BELOW]:
    ...
    async def _send_notifications(self, alert: PriceAlert, notification: AlertNotification):
    ...
    async def _send_websocket_notification(self, notification: AlertNotification):
        ...
        try:
            ...
            logger.info(f"Sent WebSocket notification for alert {notification.alert_id}")
        except Exception as e:
    ...
    def get_alert_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            triggered_alerts = sum(alert.trigger_count for alert in self.alerts.values())
...
async def get_price_alert_system(realtime_provider: Optional[RealTimeDataProvider] = None) -> PriceAlertSystem:
