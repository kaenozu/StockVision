...
class StockRenderer:
    ...
    def render_stock(
        self, stock: Union[Stock, CurrentPrice, StockData], title: Optional[str] = None
    ):
        """単一の株価データを表示。
        ...
        if isinstance(self.formatter, JSONFormatter):
            ...
            self.console.print(
                Panel(json_output, title="株価情報 (JSON)", border_style="cyan")
            )
    ...
    def render_stock_list(
        self,
        stocks: List[Union[Stock, CurrentPrice, StockData]],
        ...
        if isinstance(self.formatter, JSONFormatter):
            ...
            self.console.print(
                Panel(json_output, title="株価一覧 (JSON)", border_style="cyan")
            )
        elif isinstance(self.formatter, TableFormatter):
            # 複数株価の場合は個別にテーブル表示
...
class WatchlistRenderer:
    ...
    def render_watchlist(
        self, watchlist_items: List[Watchlist], title: Optional[str] = None
    ):
        """ウォッチリストを表示。
        ...
        if isinstance(self.formatter, JSONFormatter):
            ...
            self.console.print(
                Panel(json_output, title="ウォッチリスト (JSON)", border_style="cyan")
            )
        elif isinstance(self.formatter, TableFormatter):
...
"""
        ...
        if stocks_with_prices:
            total_value = sum(
                float(stock.current_price) for stock in stocks_with_prices
            )
...
class PriceHistoryRenderer:
    ...
    def render_price_history(
        ...
        if not price_history:
            self.console.print(
                f"[yellow]{stock_code or '該当銘柄'}の価格履歴データがありません。[/yellow]"
            )
            return
        ...
        if isinstance(self.formatter, JSONFormatter):
            ...
            title = (
                f"価格履歴 - {stock_code} (JSON)" if stock_code else "価格履歴 (JSON)"
            )
            ...
            # コンパクト形式では最新数件のみ表示
            recent_history = (
                price_history[:10] if len(price_history) > 10 else price_history
            )
            ...
            for item in recent_history:
                ...
                date_value = getattr(item, "trading_date", None) or getattr(
                    item, "date", None
                )
    ...
    def render_price_comparison(
        ...
        avg_diff_pct = (
            (avg_diff / comparison_stats["avg_price"] * 100)
            if comparison_stats["avg_price"] > 0
            else 0
        )
        table.add_row(
            "平均終値",
        ...
        max_diff_pct = (
            (max_diff / comparison_stats["max_price"] * 100)
            if comparison_stats["max_price"] > 0
            else 0
        )
        table.add_row(
            "期間高値",
        ...
        min_diff_pct = (
            (min_diff / comparison_stats["min_price"] * 100)
            if comparison_stats["min_price"] > 0
            else 0
        )
        table.add_row(
            "期間安値",
        ...
        vol_diff_pct = (
            (vol_diff / comparison_stats["avg_volume"] * 100)
            if comparison_stats["avg_volume"] > 0
            else 0
        )
        table.add_row(
            "平均出来高",
...
class InteractiveRenderer:
    ...
    def select_from_list(
        self, items: List[str], message: str = "選択してください"
    ) -> Optional[str]:
        """リストから項目を選択させる。
        ...
        while True:
            try:
                ...
                if choice_num == 0:
                    ...
                    self.console.print(
                        f"[red]1から{len(items)}の範囲で入力してください。[/red]"
                    )
            except ValueError:
