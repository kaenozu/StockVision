...
class StockFormatter:
    ...
    def format_percentage(self, percentage: Union[Decimal, float]) -> str:
        ...
        percentage_value = float(percentage) if isinstance(percentage, Decimal) else percentage
    ...
    def get_price_color(self, price_change: Union[Decimal, float]) -> str:
        ...
        change_value = float(price_change) if isinstance(price_change, Decimal) else price_change
    ...
    def to_serializable_dict(self, obj: Any) -> Dict[str, Any]:
        ...
        Returns:
            シリアライズ可能な辞書
        """
        if isinstance(obj, (Stock, Watchlist, PriceHistory, CurrentPrice, StockData, PriceHistoryItem)):
            result = {}
            ...
            # その他の属性ベースオブジェクト
                ...
                result = {attr: getattr(obj, attr) for attr in dir(obj) if not attr.startswith('_')}
...
class JSONFormatter(StockFormatter):
    ...
    def format_price_history(self, price_history: List[Union[PriceHistory, PriceHistoryItem]]) -> str:
        """価格履歴をJSON形式でフォーマット。
...
class TableFormatter(StockFormatter):
    ...
    def format_stock(self, stock: Union[Stock, CurrentPrice, StockData]) -> Table:
        ...
        table.add_row("現在価格", Text(self.format_price(stock.current_price), style=current_price_color))
        ...
        if isinstance(stock, StockData):
            ...
            if stock.dividend_yield:
                table.add_row("配当利回り", self.format_percentage(stock.dividend_yield))
    ...
    def format_watchlist(self, watchlist_items: List[Watchlist]) -> Table:
        ...
        for item in watchlist_items:
            ...
            if stock:
                ...
                current_price_text = Text(self.format_price(stock.current_price), style=price_color)
                price_change_text = Text(
                    f"{self.format_price(stock.price_change)} ({self.format_percentage(stock.price_change_pct)})",
                    style=price_color
                )
                ...
                table.add_row(
                    ...
                    current_price_text,
                    price_change_text,
                    self.format_price(item.alert_price_high) if item.alert_price_high else "-",
                    self.format_price(item.alert_price_low) if item.alert_price_low else "-",
                    item.notes or "-"
                )
            else:
                table.add_row(
    ...
    def format_price_history(self, 
                           price_history: List[Union[PriceHistory, PriceHistoryItem]], 
                           stock_code: Optional[str] = None) -> Table:
        """価格履歴をテーブル形式でフォーマット。
    ...
    def format_summary_stats(self, price_history: List[Union[PriceHistory, PriceHistoryItem]]) -> Table:
        ...
        period_change_pct = (period_change / oldest_price * 100) if oldest_price > 0 else 0
...
class CompactFormatter(StockFormatter):
    ...
    def format_stock_compact(self, stock: Union[Stock, CurrentPrice, StockData]) -> str:
        ...
        return (f"{stock.stock_code} {stock.company_name} | "
...
# フォーマッター取得のユーティリティ関数
def get_formatter(format_type: str, console: Optional[Console] = None) -> StockFormatter:
    """指定された形式のフォーマッターを取得。
