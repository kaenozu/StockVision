    ...
    try:
        ...
        async def fetch_and_save():
            try:
                ...
                # 株式データをデータベースに保存
                try:
                    # existing_stock = storage.get_stock(stock_code) # Commented out as it's unused
                    storage.update_stock(
                        stock_code=stock_code,
                        current_price=stock_data.current_price,
                        previous_close=stock_data.previous_close,
        ...
        # watchlist_item = storage.create_watchlist_item( # Commented out as it's unused
...
def watchlist_remove(ctx, stock_code: str):
    ...
    try:
        ...
        target_items = [
            item for item in watchlist_items if item.stock_code == stock_code
        ]

        if not target_items:
            renderer.render_error(
                f"銘柄コード {stock_code} はウォッチリストにありません"
            )
            return
        ...
        for item in target_items:
            if interactive.confirm_action(
                f"銘柄 {stock_code} をウォッチリストから削除しますか？"
            ):
    ...
    try:
        ...
        async def process_stock_code(stock_code: str):
            try:
                ...
                # ウォッチリストに追加
                storage.create_watchlist_item(stock_code=stock_code, notes=notes)
        ...
        # プログレス表示付きで実行
        progress_renderer.show_spinner(
            "銘柄データを一括取得中...", lambda: asyncio.run(batch_process())
        )

        # 結果の表示
        if successful_adds:
            renderer.render_success(
                f"{len(successful_adds)} 銘柄をウォッチリストに追加しました:"
            )
...
def update(ctx, stock_code: Optional[str], update_all: bool):
    ...
    try:
        ...
        # プログレス表示付きで実行
        progress_renderer.show_spinner(
            "株価データを更新中...", lambda: asyncio.run(batch_update())
        )

        # 結果の表示
        if successful_updates:
            renderer.render_success(
                f"{len(successful_updates)} 銘柄の更新が完了しました:"
            )
...
def history(ctx, stock_code: str, days: int, show_history: bool):
    ...
    try:
        ...
        try:
            ...
            renderer.render_error(
                f"銘柄 {stock_code} のデータがデータベースにありません。まず search コマンドで取得してください。"
            )
            return
        ...
        price_history = storage.get_price_history(
            stock_code=stock_code, start_date=start_date, end_date=end_date, limit=days
        )
        ...
        if price_history:
            ...
            renderer.render_info(
                f"データベースに履歴がありません。Yahoo Financeから {days} 日分のデータを取得します..."
            )
            ...
            async def fetch_history():
                try:
                    ...
                    if history_data and history_data.history:
                        ...
                        renderer.render_error(
                            f"銘柄 {stock_code} の履歴データが取得できませんでした"
                        )
