        ...
        try:
            if data_type == "stock_data":
                ...
                background_tasks.add_task(
                    csv_data_service.sync_csv_to_database, tmp_file_path
                )
                ...
                return CSVImportResponse(
                    success=False, message=f"データタイプ '{data_type}' は未対応です"
                )
...
async def download_from_colab(
    ...
    try:
        ...
        if request.data_type == "stock_data":
            background_tasks.add_task(csv_data_service.sync_csv_to_database, local_path)

        return CSVImportResponse(
            success=True, message=f"Colabから{request.data_type}をダウンロードしました"
        )
...
async def get_csv_status():
    ...
    try:
        ...
        return {"success": True, "csv_status": status}
