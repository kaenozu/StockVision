...
from ..ml.prediction_engine import prediction_engine, ModelType, PredictionHorizon, PredictionResult
...
class PredictionRequest(BaseModel):
    ...
    horizon: str = Field(default="1d", description="Prediction horizon (1h, 1d, 1w, 1m, 3m)")
...
async def get_model_metrics(
    ...
    try:
        ...
        if model_key not in prediction_engine.model_metrics:
            raise HTTPException(status_code=404, detail=f"No metrics found for {symbol} {model_type}")
...
async def retrain_models(
    symbols: Optional[List[str]] = Query(None, description="Symbols to retrain (all if empty)"),
    background_tasks: BackgroundTasks = None
):
    """
